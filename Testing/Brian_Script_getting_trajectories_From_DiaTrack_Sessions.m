...
directory_name = uigetdir;  % open dialog box for selecting directory  
files = dir(fullfile(directory_name, '*.mat'));  % find all .mat files in directory
fileIndex = find(~[files.isdir]);  % find indicies of .mat files in directory
formatSpec = 'Completed....file %d of %d.';
for z = 1:length(files);  % loop for number of .mat files in directory
    ...
    %****WRITE TO .TXT FILE****
    fileID = fopen(fullfile(directory_name,[fileName(:,1:end-4),'1.txt']),'w');
    ...
    fprintf(fileID,'\r');
    dlmwrite(fullfile(directory_name,[fileName(:,1:end-4),'1.txt']),vertcat(firstFrame,mat),'-append','delimiter','\t')
    fclose(fileID);
    display(sprintf(formatSpec,z,length(files)))
